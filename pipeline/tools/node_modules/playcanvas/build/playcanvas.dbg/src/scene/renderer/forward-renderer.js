...
const _ForwardRenderer = class _ForwardRenderer extends Renderer {
  /**
  ...
  renderForwardPrepareMaterials(camera, renderTarget, drawCalls, sortedLights, layer, pass, viewUniformFormat) {
    ...
    for (let i = 0; i < drawCallsCount; i++) {
      ...
      if (camera === _ForwardRenderer.skipRenderCamera) {
        if (_ForwardRenderer._skipRenderCounter >= _ForwardRenderer.skipRenderAfter) {
          continue;
        }
        _ForwardRenderer._skipRenderCounter++;
      }
      if (layer) {
  ...
  renderForwardInternal(camera, preparedCalls, sortedLights, pass, drawCallback, flipFaces) {
    ...
    Debug.assert(
      attachmentCount <= 1 || device.supportsIndependentBlending,
    ...
    for (let i = 0; i < preparedCallsCount; i++) {
      ...
      if (newMaterial) {
        ...
        const blendState = attachmentCount > 1 && !material.sceneTexturesWrite ? getSingleAttachmentBlendState(material.blendState, attachmentCount) : material.blendState;
...
__publicField(_ForwardRenderer, "skipRenderCamera", null);
__publicField(_ForwardRenderer, "_skipRenderCounter", 0);
__publicField(_ForwardRenderer, "skipRenderAfter", 0);
let ForwardRenderer = _ForwardRenderer;
export {
  ForwardRenderer
};
