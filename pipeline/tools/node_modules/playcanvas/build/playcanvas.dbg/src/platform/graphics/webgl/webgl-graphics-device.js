...
class WebglGraphicsDevice extends GraphicsDevice {
  ...
  constructor(canvas, options = {}) {
     ...
     * @private
     */
    __publicField(this, "_readbackCopies", /* @__PURE__ */ new Set());
  ...
  startRenderPass(renderPass) {
    ...
    const useClearBuffers = colorBufferCount > 1 || colorBufferCount === 1 && isIntegerPixelFormat(rt._colorBuffers[0].format);
    ...
    if (useClearBuffers) {
      ...
      for (let i = 0; i < colorBufferCount; i++) {
        const colorOps2 = colorArrayOps[i];
        if (colorOps2?.clear) {
          if (!writeMasksReset) {
            this.setBlendState(BlendState.NOBLEND);
            writeMasksReset = true;
          }
          const { clearValue } = colorOps2;
          const formatInfo = pixelFormatInfo.get(rt._colorBuffers[i].format);
          const clearValueArray = formatInfo?.isUint ? _attachmentClearValueUint : formatInfo?.isInt ? _attachmentClearValueInt : _attachmentClearValue;
  ...
  readTextureAsync(texture, x, y, width, height, options) {
    ...
    return new Promise((resolve, reject) => {
      const readPromise = this.readPixelsAsync(
        x,
        y,
        width,
        height,
  ...
  setTransformFeedbackBuffers(buffers) {
    ...
    if (active) {
      if (!wasActive) {
        this.feedback ?? (this.feedback = gl.createTransformFeedback());
  ...
  applyBlendState(blendState, prevState) {
    ...
    if (!prevState || prevState.colorSrcFactor !== colorSrcFactor || prevState.colorDstFactor !== colorDstFactor || prevState.alphaSrcFactor !== alphaSrcFactor || prevState.alphaDstFactor !== alphaDstFactor) {
  ...
  setBlendState(blendState) {
    ...
    if (!currentBlendState.equals(blendState)) {
      if ((blendState.hasAttachmentOverrides || currentBlendState.hasAttachmentOverrides) && this.supportsIndependentBlending) {
