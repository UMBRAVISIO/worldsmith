...
class Texture {
  ...
  constructor(graphicsDevice, options = {}) {
     ...
     * @ignore
     */
    __publicField(this, "uploadVersion", 0);
    /** @protected */
    ...
    if (requestedSamples > 1) {
      if (graphicsDevice.isWebGPU) {
        ...
        Debug.assert(
          !this._volume && !this._cubemap && this._arrayLength === 0,
          `Multisampled texture '${this.name}' must be a 2D non-array texture.`,
          this
        );
        Debug.assert(
          !this._storage,
          `Multisampled texture '${this.name}' cannot be a storage texture.`,
          this
        );
        Debug.assert(
          !options.levels,
          `Multisampled texture '${this.name}' cannot be created with initial data, it can only be rendered into.`,
          this
        );
        Debug.assert(
          options.numLevels === void 0,
          `Multisampled texture '${this.name}' cannot use the numLevels option, it always has a single mip level.`,
          this
        );
        Debug.assert(
          isMultisampleCapablePixelFormat(this._format),
          `Multisampled texture '${this.name}' uses format ${pixelFormatInfo.get(this._format)?.name}, which does not support multisampling.`,
          this
        );
      } else {
  ...
  _validateCopy(source, options) {
    ...
    if (w <= 0 || h <= 0 || sx < 0 || sy < 0 || dx < 0 || dy < 0 || sx + w > sw || sy + h > sh || dx + w > dw || dy + h > dh) {
