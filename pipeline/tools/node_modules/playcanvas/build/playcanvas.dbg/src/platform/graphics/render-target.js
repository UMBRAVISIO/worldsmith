...
class RenderTarget {
  ...
  constructor(options = {}) {
     ...
     * @private
     */
    __publicField(this, "_resolveBuffers", null);
    /**
     ...
     * @private
     */
    __publicField(this, "_depthResolveBuffer", null);
    /**
     * @type {boolean}
     * @private
     */
     ...
     * @private
     */
    __publicField(this, "_transientColor");
    /**
     * @type {boolean}
     * @private
     */
    __publicField(this, "_transientDepth");
    /** @type {boolean} */
     ...
     * @private
     */
    __publicField(this, "_depthResolveMode", DEPTHRESOLVE_MIN);
    /**
     * @type {number}
     * @private
     */
    ...
    if (explicitMsaa) {
      ...
      Debug.call(() => {
        if (options.samples !== void 0 && options.samples !== this._samples) {
    ...
    Debug.call(() => {
      if (explicitMsaa) {
        this._colorBuffers?.forEach((colorBuffer, index) => {
          Debug.assert(
            colorBuffer.samples === this._samples,
            `RenderTarget '${this.name}': all color buffers must have the same sample count, but color buffer ${index} has ${colorBuffer.samples} samples while ${this._samples} are expected.`,
            this
          );
          const format = colorBuffer.format;
          Debug.assert(
            format !== PIXELFORMAT_DEPTH && format !== PIXELFORMAT_DEPTH16 && format !== PIXELFORMAT_DEPTHSTENCIL,
            `RenderTarget '${this.name}': a depth format texture cannot be used as a color buffer, use the depthBuffer option.`,
            this
          );
        });
        this._resolveBuffers?.forEach((resolveBuffer, index) => {
          ...
          Debug.assert(
            colorBuffer,
            `RenderTarget '${this.name}': resolveBuffer at index ${index} has no matching color buffer.`,
            this
          );
          if (!colorBuffer) return;
          Debug.assert(
            resolveBuffer.samples === 1,
            `RenderTarget '${this.name}': resolveBuffer at index ${index} must be single-sampled, but has ${resolveBuffer.samples} samples.`,
            this
          );
          Debug.assert(
            resolveBuffer.format === colorBuffer.format,
            `RenderTarget '${this.name}': resolveBuffer at index ${index} format does not match its color buffer format.`,
            this
          );
          Debug.assert(
            resolveBuffer.width === colorBuffer.width && resolveBuffer.height === colorBuffer.height,
            `RenderTarget '${this.name}': resolveBuffer at index ${index} dimensions (${resolveBuffer.width}x${resolveBuffer.height}) do not match its color buffer (${colorBuffer.width}x${colorBuffer.height}).`,
            this
          );
          Debug.assert(
            isMultisampleResolveCapablePixelFormat(colorBuffer.format),
            `RenderTarget '${this.name}': color buffer format at index ${index} cannot be hardware-resolved; omit the resolveBuffer and resolve in a shader using textureLoad.`,
            this
          );
        });
        ...
        if (depthBuffer) {
          Debug.assert(
            depthBuffer.samples === this._samples,
            `RenderTarget '${this.name}': the depth buffer must have the same sample count as the color buffers, but has ${depthBuffer.samples} samples while ${this._samples} are expected.`,
            this
          );
          if (depthBuffer.samples > 1) {
            ...
            Debug.assert(
              format === PIXELFORMAT_DEPTH || format === PIXELFORMAT_DEPTH16 || format === PIXELFORMAT_DEPTHSTENCIL,
              `RenderTarget '${this.name}': a multisampled depthBuffer must use a depth format.`,
              this
            );
          }
        }
        ...
        if (depthResolveBuffer && depthBuffer) {
          Debug.assert(
            depthResolveBuffer.samples === 1,
            `RenderTarget '${this.name}': depthResolveBuffer must be single-sampled, but has ${depthResolveBuffer.samples} samples.`,
            this
          );
          Debug.assert(
            depthResolveBuffer.format === PIXELFORMAT_R32F,
            `RenderTarget '${this.name}': depthResolveBuffer must use the PIXELFORMAT_R32F format.`,
            this
          );
          Debug.assert(
            depthResolveBuffer.width === depthBuffer.width && depthResolveBuffer.height === depthBuffer.height,
            `RenderTarget '${this.name}': depthResolveBuffer dimensions (${depthResolveBuffer.width}x${depthResolveBuffer.height}) do not match the depth buffer (${depthBuffer.width}x${depthBuffer.height}).`,
            this
          );
        }
      }
  ...
  set depthResolveMode(value) {
    Debug.assert(
      value === DEPTHRESOLVE_MIN || value === DEPTHRESOLVE_MAX || value === DEPTHRESOLVE_SAMPLE0,
      `RenderTarget '${this.name}': invalid depthResolveMode '${value}'.`,
      this
    );
