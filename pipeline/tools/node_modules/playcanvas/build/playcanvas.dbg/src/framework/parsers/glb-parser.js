...
const createMeshes = (device, gltf, bufferViews, options) => {
  ...
  const flatShadedMeshes = /* @__PURE__ */ new Set();
  const promises = [];
...
const applyFlatShading = (flatShadedMeshes, meshDefaultMaterials, meshVariants, materials) => {
  ...
  const flatShadedIndex = (index) => {
    if (!clones.has(index)) {
      const material = index === void 0 ? createMaterial({ name: "defaultGlbMaterial" }, []) : materials[index].clone();
    ...
    gltf.nodes.forEach((gltfNode, nodeIndex) => {
      if (gltfNode.hasOwnProperty("camera")) {
        ...
        if (gltfCamera) {
          ...
          const camera = process ? process(gltfCamera, nodes[nodeIndex]) : createCamera(gltfCamera, nodes[nodeIndex], app);
          if (postprocess) {
  ...
  static parse(filename, urlBase, data, device, registry, options, callback) {
    parseChunk(filename, data, (err, chunks) => {
      ...
      parseGltf(chunks.gltfChunk, (err2, gltf) => {
        ...
        createResources(device, gltf, bufferViews, textures, options, registry.loader._app).then((result) => callback(null, result)).catch((err3) => callback(err3));
      });
    });
  }
