    ...
    if (tags) {
      ...
      for (let i = 0, len = this._symbols.length; i < len; ++i) {
        ...
        if (tag && tag.outline && (tag.outline.attributes.color || tag.outline.attributes.thickness)) {
          let color2 = tag.outline.attributes.color ? colorTmp.fromString(tag.outline.attributes.color) : this._outlineColor;
          ...
          if (Number.isNaN(color2.r) || Number.isNaN(color2.g) || Number.isNaN(color2.b) || Number.isNaN(color2.a)) {
            color2 = this._outlineColor;
          }
          if (Number.isNaN(thickness)) {
            thickness = this._outlineThickness;
          }
          const outlineHash = getColorThicknessHash(color2, thickness);
        ...
        if (tag && tag.shadow && (tag.shadow.attributes.color || tag.shadow.attributes.offset || tag.shadow.attributes.offsetX || tag.shadow.attributes.offsetY)) {
          let color2 = tag.shadow.attributes.color ? colorTmp.fromString(tag.shadow.attributes.color) : this._shadowColor;
          ...
          const shadowHash = getColorOffsetHash(color2, offset);
  ...
  _updateMeshes() {
        ...
        if (!data) {
          if (CONTROL_CHARS.indexOf(char) !== -1) {
            ...
            if (!json.missingChars) {
              json.missingChars = /* @__PURE__ */ new Set();
            }
    ...
    for (let i = 0; i < this._meshInfo.length; i++) {
      ...
      for (const line in this._meshInfo[i].lines) {
        ...
        for (let quad2 = prevQuad; quad2 <= index; quad2++) {
          const qoffset = justified ? hoffset + gapWidth * this._meshInfo[i].gapIndices[quad2] : hoffset;
          this._meshInfo[i].positions[quad2 * 4 * 3] += qoffset;
          this._meshInfo[i].positions[quad2 * 4 * 3 + 3] += qoffset;
          this._meshInfo[i].positions[quad2 * 4 * 3 + 6] += qoffset;
          this._meshInfo[i].positions[quad2 * 4 * 3 + 9] += qoffset;
          this._meshInfo[i].positions[quad2 * 4 * 3 + 1] += voffset;
          this._meshInfo[i].positions[quad2 * 4 * 3 + 4] += voffset;
          this._meshInfo[i].positions[quad2 * 4 * 3 + 7] += voffset;
          this._meshInfo[i].positions[quad2 * 4 * 3 + 10] += voffset;
        }
        if (this._rtl) {
          for (let quad2 = prevQuad; quad2 <= index; quad2++) {
            const idx = quad2 * 4 * 3;
