...
export class StandardMaterial extends Material {
	...
	/** The ambient color of the material, specified in sRGB color space. This color value is 3-component (RGB), where each component is between 0 and 1. */
	set ambient(arg: Color);
	get ambient(): Color;

	/** Defines amount of anisotropy. Requires {@link enableGGXSpecular} is set to true. - When anisotropyIntensity == 0, specular is isotropic. - Specular anisotropy increases as anisotropyIntensity value increases to maximum of 1. */
	...
	/** The diffuse color of the material, specified in sRGB color space. This color value is 3-component (RGB), where each component is between 0 and 1. Defines basic surface color (aka albedo). */
	...
	/** The emissive color of the material, specified in sRGB color space. This color value is 3-component (RGB), where each component is between 0 and 1. */
	...
	/** Enables GGX specular. Also enables {@link anisotropyIntensity} parameter to set material anisotropy. */
	...
	/** Defines the glossiness of the material from 0 (rough) to 1 (shiny). Materials imported from glTF enable {@link StandardMaterial#glossInvert}, which reverses this: on those materials gloss holds roughness, so 0 is shiny and 1 is rough. */
	...
	/** The height map of the material (default is null). Used for a view-dependent parallax effect. The texture must represent the height of the surface where darker pixels are lower and lighter pixels are higher, with {@link heightMapBase} selecting the value that sits at the level of the original geometry. It is recommended to use it together with a normal map. Note that the parallax offset is applied to all other maps of the material, so the height map should use the same tiling and offset as those maps. */
	...
	/** Height map multiplier (default is 1). Affects the strength of the parallax effect. A value of 1 displaces the texture by up to 5% of a UV tile, so useful values are typically in the 0 to 2 range. */
	...
	/** Used to specify whether opacity is dithered, which allows transparency without alpha blending. Can be: - {@link DITHER_NONE}: Opacity dithering is disabled. - {@link DITHER_BAYER2}: Opacity is dithered using a Bayer 2 matrix. - {@link DITHER_BAYER4}: Opacity is dithered using a Bayer 4 matrix. - {@link DITHER_BAYER8}: Opacity is dithered using a Bayer 8 matrix. - {@link DITHER_BAYER16}: Opacity is dithered using a Bayer 16 matrix. - {@link DITHER_BLUENOISE}: Opacity is dithered using a blue noise. - {@link DITHER_IGNNOISE}: Opacity is dithered using an interleaved gradient noise. Defaults to {@link DITHER_NONE}. */
	set opacityDither(arg: string);
	get opacityDither(): string;

	/** Used to specify whether shadow opacity is dithered, which allows shadow transparency without alpha blending. Can be: - {@link DITHER_NONE}: Opacity dithering is disabled. - {@link DITHER_BAYER2}: Opacity is dithered using a Bayer 2 matrix. - {@link DITHER_BAYER4}: Opacity is dithered using a Bayer 4 matrix. - {@link DITHER_BAYER8}: Opacity is dithered using a Bayer 8 matrix. - {@link DITHER_BAYER16}: Opacity is dithered using a Bayer 16 matrix. - {@link DITHER_BLUENOISE}: Opacity is dithered using a blue noise. - {@link DITHER_IGNNOISE}: Opacity is dithered using an interleaved gradient noise. Defaults to {@link DITHER_NONE}. */
	set opacityShadowDither(arg: string);
	get opacityShadowDither(): string;

	/** Used to specify whether specular and reflections are faded out using {@link opacity}. Default is true. When set to false use {@link alphaFade} to fade out materials. */
	...
	/** The specular color of the material, specified in sRGB color space. This color value is 3-component (RGB), where each component is between 0 and 1. Defines surface reflection/specular color. Affects specular intensity and tint. */
	...
	/** The specular color of the sheen (fabric) microfiber structure, specified in sRGB color space. This color value is 3-component (RGB), where each component is between 0 and 1. */
	set sheen(arg: Color);
	get sheen(): Color;
	...
	/** Apply tonemapping (as configured via {@link CameraComponent#toneMapping}). Defaults to true. */
     ...
     */
    set useGammaTonemap(value: boolean);
    /**
